...
const FAILED_MESSAGE = "Thanh toán thất bại. Vui lòng thử lại.";
...
export const CheckoutPage: React.FC = () => {
  ...
  const scenario = useMemo(() => {
    ...
    if (q === "failed" || q === "timeout") return q;
    return "";
  }, [location.search]);
    ...
    pollIntervalRef.current = window.setInterval(async () => {
      try {
        ...
        if (pollAttemptsRef.current >= MAX_POLL_ATTEMPTS) {
          stopPolling();
          setPaying(false);
          setPayError(TIMEOUT_MESSAGE);
          return;
        }

        const updatedReg = await paymentService.getRegistrationStatus(id);
        setRegistration(updatedReg);

        if (updatedReg.status === "confirmed" || updatedReg.status === "cancelled") {
          stopPolling();
          setPaying(false);
        }
      } catch {
        // Polling errors are tolerated; timeout guard handles stuck flows.
      }
    }, POLL_EVERY_MS);
  ...
  const handlePayment = async () => {
    ...
    try {
      const result = await paymentService.processPayment(id, options);
      if (result.registration) {
        setRegistration(result.registration);
      }
      if (result.status !== "pending" && result.registration?.status === "confirmed") {
        setPaying(false);
      }
    } catch (err: unknown) {
      setPaying(false);
      ...
      setPayError("Đã xảy ra lỗi khi thanh toán.");
    }
  };
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="p-5">
              ...
              <div className="space-y-1.5 text-sm text-gray-600">
                ...
                <p className="font-medium text-gray-900">
                  ...
                  <span className="ml-1 text-xl text-primary-700">{formatPrice(workshop.price)}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        {isConfirmed ? (
          <div className="flex flex-col items-center py-6 text-center">
            ...
            <h3 className="mb-2 text-xl font-bold text-gray-900">Thanh toán thành công!</h3>
            <p className="mb-6 text-sm text-gray-600">Mã QR tham dự của bạn đã được xuất.</p>

            {registration.qrCode && (
              <div className="mb-8">
                <QrCodeDisplay value={registration.qrCode} label="Mã điểm danh sự kiện" />
              </div>
            )}
          ...
          <div className="space-y-6">
            <div className="rounded-lg border border-blue-100 bg-blue-50 p-4">
              <h3 className="mb-1 text-sm font-semibold text-blue-900">Chỗ của bạn đã được giữ!</h3>
